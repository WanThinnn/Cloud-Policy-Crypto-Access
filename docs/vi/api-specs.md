...
### 1.4. Hệ Thống Log Chuyên Sâu (Granular SIEM Audit Logging)
Để đáp ứng yêu cầu compliance của doanh nghiệp, hệ thống liên tục giám sát mọi hoạt động của người dùng, đánh giá policy và các sự kiện hệ thống. Thay vì ghi vào một file log nguyên khối (monolithic), hệ thống phân luồng và trích xuất thành 7 file JSON độc lập (`crypto-access-auth.json`, `crypto-access-storage.json`, `crypto-access-policy.json`, `crypto-access-user.json`, `crypto-access-attributes.json`, `crypto-access-audit.json`, `crypto-access-system.json`). 
Mỗi dòng log JSON được làm giàu dữ liệu (enrichment) bằng các thông tin ngữ cảnh bảo mật như `user.id` và `user.name`, giúp định dạng chuẩn xác và sẵn sàng để tích hợp (ingest) ngay lập tức vào các nền tảng SIEM (Security Information and Event Management) bên ngoài như Splunk, Wazuh, hay ELK Stack để phân tích tự động.
