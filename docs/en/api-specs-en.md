...
| GET/POST | `/files/`                      | Main API for File Upload. POST data includes `file` and `policy` (Desired CP-ABE policy). Plaintext data is immediately streamed to ClamAV for a **Synchronous Early-Rejection** scan. If malware is detected, the upload is instantly aborted (HTTP 400). If safe, the file is chunk-hashed, encrypted with AES, and the AES key is encrypted with CP-ABE before saving to Supabase. Simultaneously, database fields (filename, path, metadata) are encrypted using AES-256-GCM before SQL insertion. | Yes              |
