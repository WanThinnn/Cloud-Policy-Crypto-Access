...
- **Path Obfuscation**: The physical file names stored in Cloud Storage (Supabase, S3, GCS, or Local) are randomized UUID strings, bearing no relation to the original filename or business logic.
...
| GET/POST | `/files/`                      | Main API for File Upload. POST data includes `file` and `policy` (Desired CP-ABE policy). Plaintext data is immediately streamed to ClamAV for a **Synchronous Early-Rejection** scan. If malware is detected, the upload is instantly aborted (HTTP 400). If safe, the file is chunk-hashed, encrypted with AES, and the AES key is encrypted with CP-ABE before saving to the configured Storage (Supabase/S3/GCS/Local). Simultaneously, database fields (filename, path, metadata) are encrypted using AES-256-GCM before SQL insertion. | Yes              |
...
| DELETE   | `/files/{id}/`                 | Soft-delete a file (move to Trash). Permanent deletion available for Admin/Owner.                                                                                                                | File Owner/Admin |
| POST     | `/files/clipboard_action/`     | Copy, Cut & Paste files/folders. Supports duplicate detection (auto-rename) and ABAC permission checks on both source and destination.                                                            | Yes              |
| POST     | `/files/rename/`               | Rename a file or folder. Supports encrypted field updates with blind index recalculation.                                                                                                         | Data Owner/Admin |
| POST     | `/files/create_folder/`        | Create a new folder (virtual directory via `.folder` placeholder).                                                                                                                                | Yes              |
| GET      | `/files/trash/`                | List all soft-deleted files in Trash for the current user or all users (Admin).                                                                                                                   | Yes              |
| POST     | `/files/restore/`              | Restore a soft-deleted file from Trash.                                                                                                                                                           | File Owner/Admin |
| DELETE   | `/files/permanent_delete/`     | Permanently delete a file from Trash and Cloud Storage.                                                                                                                                           | File Owner/Admin |
| GET      | `/files/{id}/versions/`        | List all versions of a file.                                                                                                                                                                      | Yes              |
| POST     | `/files/{id}/assign_access/`   | Assign CP-ABE access policy and grant specific users access to a file or folder. Supports intelligent policy combination (OR-merge re-encryption).                                                | File Owner/Admin |
