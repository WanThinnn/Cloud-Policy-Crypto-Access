...
if [ ! -f "$PKI_DIR/ca.crt" ]; then
    ...
    echo "extendedKeyUsage=serverAuth" > $PKI_DIR/server_ext.cnf
...
if [ ! -f "$CONFIG_DIR/server.conf" ]; then
    cp $CONFIG_DIR/server.conf.template $CONFIG_DIR/server.conf
fi

# Configure and start dnsmasq for local domain resolution
DOMAIN_NAME=${DOMAIN_NAME:-"cyberfortress.local"}
NGINX_IP=$(getent hosts nginx_server | awk '{print $1}')
if [ -n "$NGINX_IP" ]; then
    echo "address=/$DOMAIN_NAME/$NGINX_IP" > /etc/dnsmasq.conf
    echo "server=8.8.8.8" >> /etc/dnsmasq.conf
    echo "Starting dnsmasq..."
    dnsmasq
else
    echo "Warning: Could not resolve nginx_server, skipping dnsmasq setup."
fi
