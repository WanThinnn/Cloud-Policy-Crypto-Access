...
- **Asynchronous ClamAV Integration**: Uploaded files are scanned by a dedicated ClamAV container utilizing an asynchronous background thread. It streams in-memory plaintext chunks, guaranteeing zero-delay upload responses while actively isolating detected malware.
...
- If `USE_EXTERNAL_TLS=True` (default), open your web browser and navigate to: **`https://localhost`** or **`https://cloudsafe.cyberfortress.local`** (if you configured your `hosts` file).
