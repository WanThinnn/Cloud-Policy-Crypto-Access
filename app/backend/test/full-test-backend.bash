...
curl -s "http://192.168.1.2:5000/api/super-admin/schema/attributes/role/valid-values" \
  -H "Authorization: Bearer $ADMIN_TOKEN" | jq

echo "Getting valid values for 'department' attribute..."
curl -s "http://192.168.1.2:5000/api/super-admin/schema/attributes/department/valid-values" \
  -H "Authorization: Bearer $ADMIN_TOKEN" | jq

# Refresh schemas
echo "Refreshing schemas..."
curl -s -X POST "http://192.168.1.2:5000/api/super-admin/schema/refresh" \
  -H "Authorization: Bearer $ADMIN_TOKEN" | jq
...
USER_IDS=$(curl -s "http://192.168.1.2:5000/api/super-admin/users" \
  -H "Authorization: Bearer $ADMIN_TOKEN" | \
  python3 -c "
import json, sys
...
for USER_ID in $USER_IDS; do
  ...
  curl -s -X POST "http://192.168.1.2:5000/api/ca/users/$USER_ID/private-key" \
    -H "Authorization: Bearer $ADMIN_TOKEN" \
    -H "Content-Type: application/json" | jq
done
...
curl -s -X POST "http://192.168.1.2:5000/api/abac/setup-corporate-policies" \
  -H "Authorization: Bearer $ADMIN_TOKEN" \
  -H "Content-Type: application/json" | jq
...
USER_TOKEN=$(curl -s -X POST "http://192.168.1.2:5000/api/auth/login" \
  -H "Content-Type: application/json" \
  -d '{"username": "'"$TEST_USER_ID"'", "password": "User123!@#"}' | \
  python3 -c "import json,sys; print(json.load(sys.stdin).get('access_token',''))" 2>/dev/null)
...
if [ ! -z "$USER_TOKEN" ]; then
  ...
  curl -s "http://192.168.1.2:5000/api/files/" \
...
if [ ! -z "$USER_TOKEN" ]; then
  ...
  UPLOAD_RESULT=$(curl -s -X POST "http://192.168.1.2:5000/api/files/upload" \
    -H "Authorization: Bearer $USER_TOKEN" \
...
" 2>/dev/null)
  ...
  if [ ! -z "$FILE_ID" ]; then
    ...
    curl -s -X POST "http://192.168.1.2:5000/api/files/$FILE_ID/versions" \
      -H "Authorization: Bearer $USER_TOKEN" \
      -F "file=@version_test_v2.txt" | jq
    
    # List file versions
    echo "Listing file versions..."
    curl -s "http://192.168.1.2:5000/api/files/$FILE_ID/versions" \
      -H "Authorization: Bearer $USER_TOKEN" | jq
    
    # Test version download
    echo "Testing version download..."
    curl -s "http://192.168.1.2:5000/api/files/$FILE_ID/versions/1/download" \
      -H "Authorization: Bearer $USER_TOKEN" > downloaded_v1.txt
    
    curl -s "http://192.168.1.2:5000/api/files/$FILE_ID/versions/2/download" \
...
if [ ! -z "$USER_TOKEN" ]; then
  ...
  curl -s -X POST "http://192.168.1.2:5000/api/abac/check-corporate-access" \
    -H "Authorization: Bearer $USER_TOKEN" \
    -H "Content-Type: application/json" \
    -d '{
      "resource_type": "file",
...
curl -s "http://192.168.1.2:5000/api/health" | jq
curl -s "http://192.168.1.2:5000/api/ca/health" \
  -H "Authorization: Bearer $ADMIN_TOKEN" | jq
curl -s "http://192.168.1.2:5000/api/files/health" \
  -H "Authorization: Bearer $USER_TOKEN" | jq
curl -s "http://192.168.1.2:5000/api/abac/health" \
  -H "Authorization: Bearer $ADMIN_TOKEN" | jq
